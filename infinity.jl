...
@generated function unpack(io::IO, ::Type{T}) where{T}
	...
	code = Expr[]
	fieldvars = [Symbol("read_"*string(fn)) for fn in fieldnames(T)]
	...
	for (fn, ft, fv) in zip(fieldnames(T), fieldtypes(T), fieldvars)
		push!(code, :($fv = readfield(io, $ft, T, Val($(QuoteNode(fn))))))
	end
	expr = if T <: Tuple
...
# end
# @with_kw mutable struct ITM_hdr{S}
...
# end
# # Pack.field_vector_prop(DLG_hdr, :st_triggers, :offset_state_triggers,
# # 	:number_state_triggers)
# # Pack.field_vector_prop(DLG_hdr, :tr_triggers, :offset_transition_triggers,
# # 	:number_transition_triggers)
# # Pack.field_vector_prop(DLG_hdr, :actions, :offset_actions, :number_actions)
# 
